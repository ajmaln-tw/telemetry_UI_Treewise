...
import { Circle, FeatureGroup, MapContainer, Marker, Polyline, Popup, TileLayer, Tooltip, useMapEvent } from "react-leaflet";
...
const CustomMap = (props) => {
    ...
    const { mapJourney = [], title = "", height = "50vh", width = "100%", destination = "", startingPoint = "",
        requestInProgress = false, icon, zoom = 5, borderRadius = "15px" } = props;
                ...
                <div style={{ height, width, display: "flex", justifyContent: "center" }}>
                    ...
                    >
                        ...
                        <FeatureGroup pathOptions={{ color: "royalblue" }}>
                            ...
                            <Circle center={mapJourney[0] || [51.51, -0.1]} radius={animationPath} />
                        </FeatureGroup>
                        <Polyline
                        ...
                        {mapJourney.map((ele, i) => <div key={i}>
                            <Popup position={ele}>{ele[0]}</Popup>
                        </div>
                        )
                        }
                    </MapContainer>
